...
buck run @mode/opt <path-to-framework_overhead_benchmark>:framework_overhead_benchmark --
 --op add_op
buck run @mode/opt <path-to-framework_overhead_benchmark>:framework_overhead_benchmark --
 --op add_op --use-throughput-benchmark
"""
...
def benchmark_simple_fn(args, config, module_config, module_type, result):
    ...
    Args:
        ...
        module_config:  module_config which contains the op and the number of
                    parameters that op takes.
    ...
    result_key = f_name
    module = WrapperModule(module_type, module_config)
...
def main():
    ...
    if args.op == "add_op":
        num_params = 2
        module_config = ModuleConfig(add_tensors_loop, None, num_params)
