...
test_python_shard() {
  ...
  time python test/run_test.py --verbose --exclude-distributed-tests --shard "$1" "$NUM_TEST_SHARDS"
...
checkout_install_torchbench() {
  local commit
  commit=$(cat .ci/pins/torchbench.txt)
  ...
  pip install -r .ci/pins/huggingface-requirements.txt
