...
#define TORCH_SDT(name, ...) \
  TORCH_SDT_PROBE_N(         \
      pytorch, name, 0, TORCH_SDT_NARG(0 __VA_OPT__(,) __VA_ARGS__) __VA_OPT__(,) __VA_ARGS__)
...
#define TORCH_SDT_WITH_SEMAPHORE(name, ...) \
  TORCH_SDT_PROBE_N(                        \
      pytorch, name, 1, TORCH_SDT_NARG(0 __VA_OPT__(,) __VA_ARGS__) __VA_OPT__(,) __VA_ARGS__)
