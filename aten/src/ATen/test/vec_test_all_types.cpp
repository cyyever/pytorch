...
// NOTE: Because SVE uses ACL logic, the precision changes, hence the adjusted tolerance.
    ...
    TYPED_TEST(Exponents, ExpU20) {
        ...
        const UVT tolerance = getDefaultTolerance<UVT>();

        auto test_case = TestingCase<vec>::getBuilder()
            .addDomain(CheckWithinDomains<UVT>{{{-100, 100}}, true, tolerance})
            .setTrialCount(65536)
            .setTestSeed(TestSeed());
        ...
        for (VT value : {
            ...
            auto input = vec{value};
            auto expected = vec{VT(std::exp(value))};
            auto actual = input.exp_u20();
