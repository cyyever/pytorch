...
static inline CuFFTParamsLRUCache& cufft_get_plan_cache(
...
int64_t cufft_get_plan_cache_max_size_impl(DeviceIndex device_index) {
  TORCH_CHECK(
      0 <= device_index &&
          device_index < at::detail::getCUDAHooks().deviceCount(),
      "cufft_get_plan_cache_max_size: expected 0 <= device_index < ",
      at::detail::getCUDAHooks().deviceCount(),
...
static const Tensor& _exec_fft(
  ...
  if (C10_UNLIKELY(!pctx)) {
    ...
    // the current context
    TORCH_WARN_ONCE(
        "Attempting to run cuFFT, but there was no current CUDA context! Attempting to set the primary context...");
