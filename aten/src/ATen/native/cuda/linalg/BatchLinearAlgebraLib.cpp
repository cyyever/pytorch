...
void ldl_factor_cusolver(
  ...
  if (LD.is_complex()) {
    TORCH_CHECK(
        !hermitian,
        "torch.linalg.ldl_factor: complex tensors with hermitian=True flag are not supported on CUDA.");
  }
...
void lu_factor_looped_cusolver(const Tensor& self, const Tensor& pivots, const Tensor& infos, bool get_pivots) {
  ...
  // Necessary because cuSOLVER uses nan for outputs that correspond to 0 for non-pivoted LU.
