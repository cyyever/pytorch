...
void ldl_factor_kernel(
  ...
  switch (preferred_backend) {
    case at::LinalgBackend::Cusolver:
       { ldl_factor_cusolver(
          LD, pivots, info, upper, hermitian); return;
}
    case at::LinalgBackend::Magma:
       { ldl_factor_magma(LD, pivots, info, upper, hermitian); return;
}
    default:
...
static void geqrf_kernel(const Tensor& input, const Tensor& tau) {
  ...
  auto geqrf_cusolver_backend = [](const Tensor& input, const Tensor& tau) {
      ...
      if (input.size(-2) <= 256 && batchCount(input) >= std::max<int64_t>(2, input.size(-2) / 16)) {
        geqrf_batched_cublas(input, tau); return;
      } else {
        geqrf_cusolver(input, tau); return;
      }
      geqrf_batched_cublas(input, tau); return;
  };
  ...
  // - ?geqrf2_gpu gives correct R, but doesn't allow computation of Q via ?orgqr_gpu
    case at::LinalgBackend::Magma:
       { geqrf_magma(input, tau); return;
}
    case at::LinalgBackend::Cusolver:
    default:
       { geqrf_cusolver_backend(input, tau); return;
}
  }
#else
  ...
  switch (preferred_backend) {
    ...
    default:
      ...
      // It dispatches to cuSOLVER for CUDA inputs if USE_LINALG_SOLVER is defined
       { linalg_lstsq_gels(a, b, infos); return;
}
  }
#else
