...
struct cublasCommonArgs {
  cublasCommonArgs(const Tensor& mat1, const Tensor& mat2, Tensor& c) {
    ...
    mata = prepare_matrix_for_cublas(
        transpose_result ? mat2 : mat1, transpose_mat1, transpose_result);
    matb = prepare_matrix_for_cublas(
        ...
        [&] {
          ...
          if (tuning_ctx->IsTunableOpEnabled()) {
            ...
            at::cuda::blas::gemm_and_bias<scalar_t>(
                ...
                args.ldb,
                // This condition is needed for mm case on ROCm for hipblasLt
                // path. Passing the bias ptr as null to avoid accuracy issues
                // for mm case.
                (&result != &self) ? self.const_data_ptr<scalar_t>() : nullptr,
 ...
 const Tensor& result) {
  ...
  if (mat.numel() == 0) {
    ...
    if (!result.is_same(*self_) &&
        betaval !=
            0.0) { // if beta is 0, result contents will be zeroed later
                   //  NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
...
// assumbed to be equal to 1.0 by default. If output matrix type is 16 or 32-bit
...
//    - `scale_a`: a scalar or 1-dimensional tensor with the inverse scale of
//    `mat1`, only needed if `mat1` is a float8 type
//    - `scale_b`: a scalar or 1-dimensional tensor with the inverse scale of
//    `mat2`, only needed if `mat2` is a float8 type
...
Tensor& _scaled_mm_out_cuda(
  ...
  ScalingType scaling_choice =
      get_scaling_type(scale_a, scale_b, mat1.size(0), mat2.size(1));
  ...
  TORCH_CHECK(
      mat1.scalar_type() != ScalarType::Float8_e5m2 ||
          mat2.scalar_type() != ScalarType::Float8_e5m2,
  ...
  if (scaling_choice == ScalingType::RowWise) {
    TORCH_CHECK(
        out.dtype() == kBFloat16,
        "Only bf16 high precsion output types are supported for row-wise scaling.");
    at::cuda::detail::f8f8bf16_rowwise(
        mat1, mat2, scale_a, scale_b, bias, use_fast_accum, out);
    return out;
  }
  ...
  if (mat1.scalar_type() == ScalarType::Float8_e4m3fnuz) {          \
    ...
    AT_DISPATCH_V2(
        ...
        AT_WRAP([&] {
          ...
          params.bias_dtype = bias       ? bias->scalar_type()
              : isFloat8Type(out_dtype_) ? at::ScalarType::Half
                                         : out_dtype_;
          ...
          if (transa_ && transb_) {
            TUNABLE_DISPATCH(
                at::cuda::tunable::BlasOp::T, at::cuda::tunable::BlasOp::T)
          } else if (transa_ && !transb_) {
            TUNABLE_DISPATCH(
                at::cuda::tunable::BlasOp::T, at::cuda::tunable::BlasOp::N)
          } else if (!transa_ && transb_) {
            TUNABLE_DISPATCH(
                at::cuda::tunable::BlasOp::N, at::cuda::tunable::BlasOp::T)
          } else if (!transa_ && !transb_) {
            TUNABLE_DISPATCH(
                at::cuda::tunable::BlasOp::N, at::cuda::tunable::BlasOp::N)
          } else {
            TORCH_CHECK(false, "unreachable");
          }
        }),
