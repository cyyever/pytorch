...
def register_dataclass(
    ...
    *,
    ...
    def _unflatten_fn(values: Iterable[Any], context: Context) -> Any:
        ...
        return cls(**dict(zip(flat_names, values, strict=True)), **dict.fromkeys(none_names))
    ...
    def _flatten_fn_with_keys(obj: Any) -> tuple[list[Any], Context]:
        ...
        return [(GetAttrKey(k), v) for k, v in zip(flat_names, flattened, strict=True)], flat_names
...
def _namedtuple_flatten_with_keys(
    ...
    # pyrefly: ignore  # bad-return
    return (
        [(GetAttrKey(field), v) for field, v in zip(context._fields, values, strict=True)],
        context,
    )
...
def _defaultdict_flatten_with_keys(
    ...
    # pyrefly: ignore  # bad-return
    return [(MappingKey(k), v) for k, v in zip(dict_context, values, strict=True)], context
