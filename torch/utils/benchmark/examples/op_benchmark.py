...
def run(n, stmt, fuzzer_cls):
    ...
    raw_results = []
    for i, (float_values, int_values) in enumerate(zip(float_iter, int_iter, strict=False)):
    ...
    for results, spacer in [(parsed_results[:10], "..."), (parsed_results[-10:], "")]:
        for t_float, t_int, rel_diff, descriptions in results:
            ...
            for t_str, (name, shape, order, steps) in zip(time_str, descriptions, strict=False):
