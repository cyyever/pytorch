...
class FuzzedTensor:
    ...
    def _make_tensor(self, params, state):
        ...
        slices = [slice(0, size * step, step) for size, step in zip(size, steps, strict=False)]
    ...
    def _get_size_and_steps(self, params):
        ...
        allocation_size = tuple(size_i * step_i for size_i, step_i in zip(size, steps, strict=False))
