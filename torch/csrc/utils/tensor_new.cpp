...
ScalarType infer_scalar_type(PyObject* obj) {
  ...
  if (is_numpy_available()) {
    if (PyArray_Check(obj)) {
      return numpy_dtype_to_aten(PyArray_TYPE(reinterpret_cast<PyArrayObject*>(obj)));
    }
    if (PyArray_CheckScalar(obj)) {
      THPObjectPtr arr(PyArray_FromScalar(obj, nullptr));
      return numpy_dtype_to_aten(PyArray_TYPE(reinterpret_cast<PyArrayObject*>(arr.get())));
    }
  }
#endif
...
Tensor tensor_fromDLPack(PyObject* data) {
  ...
  if (PyCapsule_IsValid(
          data, at::DLPackTraits<DLManagedTensorVersioned>::capsule)) {
    auto versioned = static_cast<DLManagedTensorVersioned*>(PyCapsule_GetPointer(
        data, at::DLPackTraits<DLManagedTensorVersioned>::capsule));
    ...
    auto managed = static_cast<DLManagedTensor*>(PyCapsule_GetPointer(
