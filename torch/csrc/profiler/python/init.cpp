...
static int THPCapturedTraceback_traverse(
  ...
  return (reinterpret_cast<THPCapturedTraceback*>(self))
      ->data->traversePython(reinterpret_cast<int (*)(void*, void*)>(visit), arg);
}
...
PyObject* RecordFunctionFast_new(
    ...
    PyObject* args,
    PyObject* kwargs) {
  RecordFunctionFast* self = reinterpret_cast<RecordFunctionFast*>(subtype->tp_alloc(subtype, 0));
  if (self != nullptr) {
    self->name = nullptr;
...
void initPythonBindings(PyObject* module) {
  ...
  PyModule_AddObject(
      m.ptr(), "CapturedTraceback", reinterpret_cast<PyObject*>(&THPCapturedTracebackType));
  m.def(
      "gather_traceback",
  ...
  m.def("symbolize_tracebacks", [](const py::list& tbs) {
    ...
    for (py::handle tb : tbs) {
      tb_ptrs.emplace_back((reinterpret_cast<THPCapturedTraceback*>(tb.ptr()))->data.get());
    }
  ...
  RecordFunctionFast_Type.tp_dealloc = static_cast<destructor>(RecordFunctionFast_dealloc);
