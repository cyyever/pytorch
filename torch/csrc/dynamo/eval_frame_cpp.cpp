...
static py::object dynamo_call_callback(
  ...
  frame->locals = reinterpret_cast<PyObject*>(framelocals_mapping_to_dict(locals));
  ...
  py::object result = callback(
      py::handle(reinterpret_cast<PyObject*>(frame)), cache_entry_obj, py::handle(frame_state));
  Py_DECREF(frame);
  return result;
}
