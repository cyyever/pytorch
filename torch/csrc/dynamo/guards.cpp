...
static PyMethodDef TensorGuards_methods[] = {
    {"check",
     ...
     ""},
    {"check_verbose",
     static_cast<PyCFunction>(reinterpret_cast<void*>(TensorGuards_check_verbose)),
     METH_VARARGS | METH_KEYWORDS,
...
class GLOBAL_STATE : public LeafGuard {
 public:
  GLOBAL_STATE(
    ...
    _guard->init();
    owner_ = py::reinterpret_steal<py::object>(reinterpret_cast<PyObject*>(_guard));
  }
...
class GuardAccessor {
 public:
  ...
  virtual bool check_nopybind(FrameLocalsMapping* map, bool matches_dict_tag) {
    // throw std::runtime_error("fallback to python");
    // Could fallback to running check on the Python dict (lazily constructed)
    return check_nopybind(reinterpret_cast<PyObject*>(map->to_dict()), matches_dict_tag);
  }
...
class TORCH_FUNCTION_MODE_STACK : public LeafGuard {
 public:
  ...
  bool check_nopybind_template(T* value) {
    ...
    const size_t len = static_cast<size_t>(at::impl::PythonTorchFunctionTLS::stack_len());
...
class TypeDictGuardAccessor : public GuardAccessor {
 public:
  ...
  bool check_nopybind(PyObject* obj, bool matches_dict_tag = false)
      override { // borrowed ref
    PyObject* x = (reinterpret_cast<PyTypeObject*>(obj))->tp_dict; // borrowed ref
    if (x == nullptr) {
      return false;
    }
    return _guard_manager->check_nopybind(x);
...
class TypeMROGuardAccessor : public GuardAccessor {
 public:
  ...
  bool check_nopybind(PyObject* obj, bool matches_dict_tag = false)
      override { // borrowed ref
    PyObject* x = (reinterpret_cast<PyTypeObject*>(obj))->tp_mro; // borrowed ref
    return _guard_manager->check_nopybind(x);
  }
...
PyObject* torch_c_dynamo_guards_init() {
  ...
  TensorGuardsType.tp_dealloc = reinterpret_cast<destructor>(TensorGuards_dealloc);
  ...
  GlobalStateGuardType.tp_init = reinterpret_cast<initproc>(GlobalStateGuard_init);
  ...
  if (PyModule_AddObject(m, "TensorGuards", reinterpret_cast<PyObject*>(&TensorGuardsType)) < 0) {
  ...
  if (PyModule_AddObject(
          m, "GlobalStateGuard", reinterpret_cast<PyObject*>(&GlobalStateGuardType)) < 0) {
