...
ExtraState* get_extra_state(PyCodeObject* code) {
  ...
  _PyCode_GetExtra(reinterpret_cast<PyObject*>(code), extra_index, reinterpret_cast<void**>(&extra));
  return extra;
}
    ...
    if (valid) {
      try {
        ...
        if (guard_error_hook) {
          ...
          py::handle f_locals_dict = reinterpret_cast<PyObject*>(f_locals->to_dict());
