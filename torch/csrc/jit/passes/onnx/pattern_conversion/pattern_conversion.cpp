...
std::unordered_map<int64_t, ConvertedIndex> MergeSliceAndSelectToIndices(
  ...
  for (auto it = slice_and_select_nodes.rbegin();
    ...
    while (cur_dim < dim) {
      ...
      if (cur_dim - dim_offset >= static_cast<int64_t>(orig_tensor_indices.size()) ||
      ...
      } else if (cur_dim - dim_offset < static_cast<int64_t>(orig_tensor_indices.size())) {
        dim_index_map.emplace(
            std::piecewise_construct,
            std::forward_as_tuple(cur_dim),
  ...
  while (cur_dim - dim_offset < static_cast<int64_t>(orig_tensor_indices.size())) {
    dim_index_map.emplace(
        std::piecewise_construct,
        std::forward_as_tuple(cur_dim),
        std::forward_as_tuple(
