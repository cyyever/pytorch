...
def get_device_type(
    x: IRNode | OutputSpec | torch.device | str | None,
) -> str | None:
...
def is_triton(x: IRNode | torch.device | str | None) -> bool:
...
def is_cpu(x: IRNode | torch.device | str | None) -> bool:
...
class WelfordReduction(MultiOutputReduction):
    ...
    def create(  # type: ignore[override]
        ...
        def const(val: int) -> TensorBox | ShapeAsConstantBuffer:
...
class WhileLoop(ExternKernel):
    ...
    def _clone_aliased_inputs(carried_inputs: Sequence[IRNode]) -> Sequence[IRNode]:
        ...
        result: list[IRNode | TensorBox | ShapeAsConstantBuffer] = []
