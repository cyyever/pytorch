...
$0: f32[] = torch._ops.aten.empty.memory_format([], device=device(type='cpu'), pin_memory=False)""",
    ...
    def test_mode_returning_too_many_values(self) -> None:
        # pushPyOutToStack walked the returned sequence and indexed the
        # schema's return list with the same counter, so a mode returning more
        # values than the schema declares read past the end of that vector.
