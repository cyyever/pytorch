...
def tf32_off():
    ...
    with torch.backends.mkldnn.flags(deterministic=deterministic, allow_tf32=False):
        yield
...
def tf32_on(self, tf32_precision=1e-5):
    ...
    try:
        ...
        with torch.backends.mkldnn.flags(deterministic=deterministic, allow_tf32=True):
            yield
    finally:
...
class TestBasicGEMM(TestCase):
    ...
    def _test_addbmm_baddbmm(self, func, b1, b2, ref, out_tensor):
        ...
        getattr(out_tensor, func + "_")(b1, b2, beta=1.0, alpha=0.5)
        self.assertEqual(out_tensor, ref * 2.5)
        getattr(res3, func + "_")(b1, b2, beta=1.0, alpha=0.5)
        self.assertEqual(out_tensor, res3)

        self.assertEqual(
            out_tensor, getattr(torch, func)(out_tensor, b1, b2, beta=1, alpha=0)
        )
